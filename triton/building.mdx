...
### Action with API integration

**You:** _"Create an Action that integrates with the Stripe API"_

**Triton:**
- Uses the **Knowledge Store** to research API documentation and best practices
- Generates proper API authentication code
- Creates comprehensive testing flows
- Handles errors and edge cases
- Caches API research for future reference

The Knowledge Store system enables Triton to:
- Research external APIs intelligently
- Store and retrieve API integration patterns
- Generate better code with real-world API understanding
- Test API integrations comprehensively

### Action with OAuth integration

**You:** _"Create an Action that sends emails via Gmail"_

**Triton:**
1. Generates the action code with OAuth token access
2. Uses `get_oauth_token('google')` for authentication
3. Implements Gmail API calls with proper error handling
4. Prompts you to configure OAuth modifier for Google
...
- Knows which OAuth providers are available (Google, Slack, GitHub, etc.)
- Generates proper token access patterns
- Handles token refresh transparently
- Includes error handling for OAuth failures
- Prompts you to configure the OAuth modifier before execution
...
**You:** _"Create an Action that processes a batch of orders with retry logic"_

**Triton generates:**
- Action with **for loop** to iterate over orders
- **While loop** for retry logic with exponential backoff
- Proper loop state management
- Error handling for individual items

Actions now support full loop constructs (for/while) for batch processing and iterative operations.
